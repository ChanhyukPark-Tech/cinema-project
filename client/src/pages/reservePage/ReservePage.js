...
import {
    BranchContainer, BranchListContainer, BranchName, BranchTitleContainer,
    LocalContainer, LocalName,
    MovieListContainer,
    ReserveContainer, ReserveEntireContainer,
    TimeListContainer
...
function ReservePage(props) {
    ...
    return (
        <>
            <Header/>
            <Title title={"예매하기"} span={"예매하기"}/>
            <ReserveEntireContainer>

                <Steps current={step}>
                    <Step title="지역을 골라주세요" description="원하는 지역선택"/>
                    <Step title="지점을 골라주세요"/>
                    <Step title="영화를 골라주세요" description="관람을 원하는 영화 선택"/>
                    <Step title="시간을 골라주세요" description="관람을 원하는 시간 선택"/>
                </Steps>
                <ReserveContainer>
                    <BranchContainer>
                        ...
                        <div style={{display: 'flex', height: '90%'}}>
                            <LocalContainer>
                                {
                                    local.map(item => (
                                        <LocalName active={localCode === item.id} onClick={(e) => {
                                            setStep(1);
                            ...
                            <BranchListContainer>
                                {
                                    branch[localCode - 1].branches.map(item => (
                                        <BranchName active={branchCode === item.bi} onClick={e => {
                                            setStep(2)
                        ...
                        <div>
                            {
                                movies[branchCode - 1].movies.map(movie => (
                                    <ReserveMovieCard movieCode={movieCode} setMovieCode={setMovieCode} setStep={setStep} name={movie.name} grade={movie.grade}
                                                      rating={movie.rating} imgUrl={movie.imgUrl}
                                                      releasedDate={movie.releasedDate} mi={movie.mi}
                                    />
                                ))
                            }
                        </div>
                    ...
                    <TimeListContainer>
                        ...
                        <div>
                            <ReserveTimeList localCode={localCode} branchCode={branchCode} movieCode={movieCode}/>
